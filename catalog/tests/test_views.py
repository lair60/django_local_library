...
class AuthorCreateViewTest(TestCase):
    ...
    def test_form_death_date_initially_has_initial_date(self):
        login = self.client.login(username='testuser2', password='2HJ1vRV0Z&3iD')
        response = self.client.get(reverse('author-create'))
        self.assertEqual(response.status_code, 200)
    ...
    def test_redirects_to_author_detail_on_success(self):
        login = self.client.login(username='testuser2', password='2HJ1vRV0Z&3iD')
        date_of_birth = datetime.date(1985, 7, 13)
        date_of_death = datetime.date(9999, 12, 31)
        first_name = 'Luis Alberto'
        last_name = 'Inga Rivera'
        response = self.client.post(reverse('author-create'), {'first_name':first_name, 'last_name': last_name, 'date_of_birth': date_of_birth, 'date_of_death': date_of_death})
        self.assertRedirects(response, reverse('author-detail',args=[str(1)]))